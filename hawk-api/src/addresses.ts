import * as web3 from "@solana/web3.js";

export const GLOBAL_ALT = [
  "AC3t5k5PUTJirxGEHgGGQStafu1vXkvddPyqVNDrRQDt",
  "F2gRaSdfWYucAMAKv3CHReKMHvPkE3sNWrQPPPPR6UMV",
  "FQHdXaRjfKxEfqDXMs6L1gfx2wwa6idDVB8hdQuf6eSD",
  "4vMQAkkjhxrgJmqbRd4Z9YP9K7WFVAhHZn4jTJoN45Qj",
  "6KhzJ2cs85hKdLeUXS6QjQnygUY9rFUnJADJwrYp9mjM",
  "4UgEL5PQVu9MSarh8yb7U22GEZQoSkmNM3w7mZFx47aL",
  "5FrgBCVUtwD1cFw39PZxnUsBaAecZVxmaYEcvv6y7b1h",
  "FJ76CVMrHBxQxLcQkKdh9YBpwZuECsPjBLJWphwS34XF",
  "Fgxms1gyzwhF1L3pJSoAxrNkogbg9sqct7U1z6HfLv5A",
  "7vaSmpacdSv7vDjpGhBfZf3rLXjsfDkY6A12JkbNqK5Q",
  "AEAMg38ZiQHanFqFiYBQSqsmqyUHj66LZXQzEaD8YJxc",
];

export const IYF_MAIN = new web3.PublicKey("FqGg2Y1FNxMiGd51Q6UETixQWkF5fB92MysbYogRJb3P");
export const IYF_EXTENSION = new web3.PublicKey("EZiUb6ydWpR3ciizBTJ1J36KCqLyPKVjh4yZEJbs5Uno");
export const STABLEMINT_1 = new web3.PublicKey("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v");
export const STABLEMINT_2 = new web3.PublicKey("9vMJfxuKxXBoEa7rM12mYLMwTacLMLDJqHozw96WQL8i");
export const TOKEN_PROGRAM_ID = new web3.PublicKey("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA");
export const ASSOCIATED_TOKEN_PROGRAM = new web3.PublicKey("ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL");
export const SITE_FEE_OWNER = new web3.PublicKey("4K3a2ucXiGvuMJMPNneRDyzmNp6i4RdzXJmBdWwGwPEh");
export const USDC_FARM = new web3.PublicKey("7jLQhREMxXjKdpwVuN6gwsWt3BNfAg9WqbepffPbi4ww");
export const SOME_METEORA_DLMM_POOL = new web3.PublicKey("7eexH14UjhNxJe6zTT3f1Vb1E8iACsBMVaWheDEmxdT2");
export const METEORA_DLMM_PROGRAM = new web3.PublicKey("LBUZKhRxPF3XUpBCjp4YzTKgLccjZhTSDM9YuVaPwxo");
export const COMPUTE_BUDGET = new web3.PublicKey("ComputeBudget111111111111111111111111111111");
export const HS_AUTHORITY = new web3.PublicKey("dche7M2764e8AxNihBdn7uffVzZvTBNeL8x4LZg5E2c");
export const ORCA_WHIRLPOOL_PROGRAM = new web3.PublicKey("whirLbMiicVdio4qvUfM5KAg6Ct8VwpYzGff3uctyCc");
export const WSOL_MINT = new web3.PublicKey("So11111111111111111111111111111111111111112");
