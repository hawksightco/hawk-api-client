...
describe('Orca Transaction Generation', () => {
  ...
  //   await localnetTest(result, [testWalletKp, positionMintKp], [], false);
  // }, TIMEOUT);
  ...
  //   expect(result.status).toBe(200);
  //   // await simulateOrExecuteTransaction(result.data, [testWalletKp]);
  //   await localnetTest(result, [testWalletKp]);
  // }, TIMEOUT);
...
async function simulateOrExecuteTransaction(txMetadata: TransactionMetadata, signers: web3.Keypair[] = []) {
  ...
  if (executeTransaction && signers.length > 0) {
    await txMetadata.transaction.addPriorityFeeIx(connection, Math.ceil(simulation.unitsConsumed * 1.1), false, PriorityLevel.High);
...
async function localnetTest(tx: ResponseWithStatus<TransactionMetadata>, signers: web3.Keypair[], requestAirdrop: web3.PublicKey[] = [], kill: boolean = true) {
